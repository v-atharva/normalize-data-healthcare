...
# Resolve paths from this file so execution is stable from any working directory.
...
# Target schema for each output table.
SCHEMAS = {
    "DimPatient": [
        "patient_id",
        "patient_first_name",
...
# Known input date formats, in likely order.
DATE_FORMATS = [
    ("%Y-%m-%d", lambda x: True),
    ("%m/%d/%Y", lambda x: "/" in x),
    ("%d-%m-%Y", lambda x: "-" in x and len(x.split("-")) == 3),
    ("%Y/%m/%d", lambda x: "/" in x and len(x) >= 8),
]

# Visits on/after this date mark a patient as Active.
ACTIVE_START_DATE = datetime.datetime(2022, 1, 1)


class DataProcessor:
    def __init__(self, batch_size: int = 100):
        # Stores for normalized dimension/fact rows.
        ...
        # Lookup keys for generated integer IDs.
        ...
        # Seen visit IDs prevent duplicate fact rows.
        self.processed_visit_ids: Set[str] = set()

        # Visit dates are used after processing to set patient status.
    ...
    def parse_date(self, date_str: str) -> str:
        ...
        try:
            # Strip time component if present (e.g. "2022-12-28 16:17:45.753446")
            date_only = date_str.split(" ")[0].split("T")[0]

            # Try matching against known date formats
            for fmt, condition in DATE_FORMATS:
                if condition(date_only):
                    try:
                        return datetime.datetime.strptime(date_only, fmt).strftime(
                            "%Y-%m-%d"
                        )
                    except ValueError:
                        continue
    ...
    def _process_row(self, row: Dict[str, str]) -> None:
        ...
        if visit_date:
            ...
            print(
                f"Warning: could not parse visit date '{visit_datetime}' for patient {patient_id}"
            )
    ...
    def _process_patient(self, row: Dict[str, str], patient_id: str) -> None:
        ...
        self.patients[patient_id] = {
            ...
            "patient_status": "Active",  # Placeholder; updated after all visits are processed.
        }
...
def main():
    ...
    print("Starting healthcare data transformation...\n")
